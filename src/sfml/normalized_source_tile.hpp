...
// removed destination X,Y,Z, animation_state and blend_mode and blend_other.
// i noticed some animations in glform1 didn't have matching blends
struct normalized_source_animated_tile
{
   public:
     ...
     normalized_source_animated_tile()                                         = default;
     constexpr auto operator<=>(const normalized_source_animated_tile &) const = default;
     normalized_source_animated_tile(const is_tile auto &tile)
     {
          m_tex_id_buffer = m_tex_id_buffer.with_id(tile.texture_id()).with_depth(tile.depth()).with_draw(tile.draw());
          m_palette_id    = m_palette_id.with_id(tile.palette_id());
          assert(std::cmp_less(tile.source_x(), (std::numeric_limits<std::uint8_t>::max)()));
          assert(std::cmp_less(tile.source_y(), (std::numeric_limits<std::uint8_t>::max)()));
          m_source_xy    = Point<std::uint8_t>{ static_cast<std::uint8_t>(tile.source_x()), static_cast<std::uint8_t>(tile.source_y()) };
     ...
     normalized_source_animated_tile &operator=(const is_tile auto &tile)
     {
          m_tex_id_buffer = m_tex_id_buffer.with_id(tile.texture_id()).with_depth(tile.depth()).with_draw(tile.draw());
          m_palette_id    = m_palette_id.with_id(tile.palette_id());
          assert(std::cmp_less(tile.source_x(), (std::numeric_limits<std::uint8_t>::max)()));
     ...
     normalized_source_animated_tile(const normalized_source_animated_tile &)                = default;
     normalized_source_animated_tile &operator=(const normalized_source_animated_tile &)     = default;
     normalized_source_animated_tile(normalized_source_animated_tile &&) noexcept            = default;
     normalized_source_animated_tile &operator=(normalized_source_animated_tile &&) noexcept = default;
};
}// namespace open_viii::graphics::background
