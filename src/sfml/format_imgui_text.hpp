...
#include <imgui.h>
inline const auto format_imgui_text = []<std::size_t sizeT = 0xFFU, typename... T>(fmt::format_string<T...> fmt, T &&...items) {
...
inline const auto format_imgui_wrapped_text = []<std::size_t sizeT = 0xFFU, typename... T>(fmt::format_string<T...> fmt, T &&...items) {
     std::array<char, sizeT> buffer{};
     const auto              it = fmt::vformat_to_n(
       std::begin(buffer), sizeT - 1U, fmt::string_view(fmt), fmt::make_format_args(std::forward<decltype(items)>(items)...));
     *it.out = '\0';
