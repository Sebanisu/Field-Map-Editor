...
void fme::draw_window::on_im_gui_update() const
{
     ...
     const auto is_valid_float
       = [](const float f) -> bool { return !std::isnan(f) && !std::isinf(f); };
     const auto is_valid_ImVec2 = [&is_valid_float](const ImVec2 v) -> bool
                                                      ...
                                                      ff_8::Color   color2)
     {
          if (
            ...
            || !is_valid_ImVec2(window_pos) || !is_valid_ImVec2(window_size)
          ...
          if (
            m_checkerboard_framebuffer.width()
          ...
          m_fixed_render_camera.set_projection(
            0.f,
            static_cast<float>(m_checkerboard_framebuffer.width()),
            0.f,
            static_cast<float>(m_checkerboard_framebuffer.height()));
          ...
          if (!ImGuizmo::IsOver())
          {
               ImGui::InvisibleButton(
                 "##DrawWindowViewport", window_size, ImGuiButtonFlags_None);
          }
          else
          {
          ...
          ImGui::GetWindowDrawList()->AddImage(
            glengine::ConvertGliDtoImTextureId<ImTextureID>(
              m_checkerboard_framebuffer.color_attachment_id()),
            window_pos,
     ...
     const auto map_test = [&]()
     {
          return !t_map_sprite->fail()
                 && selections->get<ConfigKey::DrawMode>()
                      == draw_mode::draw_map;
     };
     const auto mim_test = [&]()
     {
          return !t_mim_sprite->fail()
                 && selections->get<ConfigKey::DrawMode>()
                      == draw_mode::draw_mim;
     };
     ...
     if (mim_test())
     {
          ...
          // ImGui::PopStyleColor(); });
          //  ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4{ clear_color.r /
          //  256.F, clear_color.g / 256.F, clear_color.b / 256.F, 0.9F });
          if (!ImGui::Begin(
                gui_labels::draw_window_title.data(), &visible, window_flags))
          {
               return;
          }
          ...
          const float scale      = std::max(
            wsize.x / static_cast<float>(img_size.x),
            wsize.y / static_cast<float>(img_size.y));
          const ImVec2 scaled_size(
            static_cast<float>(img_size.x) * scale,
            static_cast<float>(img_size.y) * scale);

          DrawCheckerboardBackground(
            screen_pos,
            scaled_size,
            (selections->get<ConfigKey::DrawPalette>() ? 0.25F * scale
                                                       : 4.F * scale),
          ...
          ImGui::GetWindowDrawList()->AddImage(
            glengine::ConvertGliDtoImTextureId<ImTextureID>(
              t_mim_sprite->get_texture()->id()),
            screen_pos,
            ImVec2{ screen_pos.x + scaled_size.x,
                    screen_pos.y + scaled_size.y });
     ...
     else if (map_test())
     {
          ...
          const auto  screen_pos  = ImGui::GetCursorScreenPos();
          const float scale
            = (std::max)(wsize.x / static_cast<float>(img_size.x),
                         wsize.y / static_cast<float>(img_size.y));
          const ImVec2 scaled_size(
            static_cast<float>(img_size.x) * scale,
            static_cast<float>(img_size.y) * scale);

          const BackgroundSettings &bg_settings
            = selections->get<ConfigKey::BackgroundSettings>();
          const auto color1 = [&]()
          {
               if (
          ...
          DrawCheckerboardBackground(
             ...
             * static_cast<float>(
               selections->get<ConfigKey::BackgroundCheckerboardScale>()))
              * scale,
            color1,
            color2);
          ...
          ImGui::GetWindowDrawList()->AddImage(
            glengine::ConvertGliDtoImTextureId<ImTextureID>(
              framebuffer.color_attachment_id(
                selections->get<ConfigKey::DrawPupuMask>() ? 1 : 0)),
            screen_pos,
            ImVec2{ screen_pos.x + scaled_size.x,
                    screen_pos.y + scaled_size.y });
...
void fme::draw_window::remove_clicked_index(std::size_t in_index) const
{
     const auto remove_result = std::ranges::remove_if(
       ...
       [&](std::size_t index) { return in_index == index; });
     ...
     if (ImGui::IsItemHovered())
     {
          ...
          if (selections->get<ConfigKey::DrawSwizzle>())
          {
               ...
               m_mouse_positions.texture_page
                 = static_cast<uint8_t>(m_mouse_positions.pixel.x / 256);
          }
          else
          {
               m_mouse_positions.texture_page
          ...
          m_mouse_positions.mouse_moved
            = (m_mouse_positions.pixel != old_pixel);
          ...
          if (m_mouse_positions.mouse_moved)
          {
               t_map_sprite->const_visit_working_tiles(
                 [&](const auto &tiles)
                 {
                      m_hovered_tiles_indices = t_map_sprite->find_intersecting(
                        tiles,
          ...
          if (ImGui::IsItemClicked(ImGuiMouseButton_Left))
          {
               ...
               else
               {
                    t_map_sprite->const_visit_working_tiles(
                      ...
                      {
                           m_clicked_tile_indices
                             = t_map_sprite->find_intersecting(
                               tiles,
                               m_mouse_positions.pixel,
                               m_mouse_positions.texture_page,
                               false,
  ...
  const float   scale) const
{
     ...
     // Drawing grid lines within the window if
     // selections->get<ConfigKey::DrawGrid>() is true
     ...
     const ImVec2 img_end
       = { screen_pos.x + scaled_size.x, screen_pos.y + scaled_size.y };

     // Calculate grid spacing
     const float grid_spacing
       = 16.0f * scale * static_cast<float>(framebuffer.scale());
     ...
     t_map_sprite->const_visit_working_tiles(
       [&](const auto &working_tiles)
       {
            ...
            const auto &animation_counts
              = t_map_sprite->working_animation_counts();

            for (const auto &indices :
                 t_map_sprite->working_conflicts().range_of_conflicts())
            {
                 const auto action = [&](const auto index)
                 {
                      assert(
                        std::cmp_less(index, std::ranges::size(working_tiles))
                        && "Index out of Range...");
                      const auto index_to_working_tile
                        = [&working_tiles](const auto i)
                      {
                      ...
                      {
                           if (auto it = similar_counts.find(working_tile);
                               it != similar_counts.end())
                           {
                                return it->second;
                           }
                           return {};
                      ...
                      {
                           if (auto it = animation_counts.find(working_tile);
                               it != animation_counts.end())
                           {
                                return it->second;
                           }
                           return {};
                        ...
                        = [&]() -> std::pair<ff_8::Color, float>
                      {
                           ...
                           if (
                             (!selections->get<ConfigKey::DrawSwizzle>()
                              && !animation_over_1
                      ...
                      // todo add hover action using the hovered_indices and
                      // change color for if similar counts >1
                 };
                 if (selections->get<ConfigKey::DrawSwizzle>())
                 {// all are drawn in the same spot so we only need to draw one.
                      if (
                        std::ranges::find(indices, m_hovered_index)
                        == std::ranges::end(indices))
                      {
                           std::ranges::for_each(
                             indices | std::ranges::views::take(1), action);
                      }
                      else
                      {
                      ...
                      // there might be different kinds of conflicts in the same
                      // location. but here we're assuming your either one or
                      // another. because we can't quite draw all the colors in
                      // the same place.
                 }
                 else
                 {
  ...
  const float   scale) const
{
     ...
     // selections->get<ConfigKey::DrawGrid>() is true
     if (
       !selections->get<ConfigKey::DrawTexturePageGrid>()
       || !selections->get<ConfigKey::DrawSwizzle>())
     {
          return;
     }
     ...
     // Calculate grid spacing
     const float grid_spacing
       = 256.0f * scale * static_cast<float>(framebuffer.scale());
  ...
  const float   scale) const
{
     ...
     // Calculate grid spacing
     const float grid_spacing
       = (selections->get<ConfigKey::DrawPalette>() ? 1.F : 16.0f) * scale;
  ...
  const float   scale) const
{
     ...
     if (
       !selections->get<ConfigKey::DrawTexturePageGrid>()
       || selections->get<ConfigKey::DrawPalette>())
     {
          return;
     }
     ...
     const float grid_spacing = [&]()
     {
          ...
          {
               spdlog::error(
                 "selections->get<ConfigKey::Bpp>().raw() = {}",
                 selections->get<ConfigKey::Bpp>().raw());
               throw;
          }
     }() * scale;
  ...
  [[maybe_unused]] const ImVec2 &screen_pos) const
{
     ...
     const ImVec2   scroll_edge_min
       = ImGui::GetCursorScreenPos() + ImVec2{ edge_threshold, edge_threshold }
         + ImVec2{ ImGui::GetScrollX(), ImGui::GetScrollY() };
     const ImVec2 scroll_edge_max
       = ImGui::GetCursorScreenPos() + ImGui::GetContentRegionAvail()
         - ImVec2{ edge_threshold, edge_threshold }
         + ImVec2{ ImGui::GetScrollX(), ImGui::GetScrollY() };
     ...
     // draw_list->AddRect(scroll_edge_min, scroll_edge_max, IM_COL32(255, 0, 0,
     // 255));
     ...
     const glm::mat4 view
       = m_fixed_render_camera.view_matrix();// identity for 2D or your actual
                                             // camera view if available

     glm::mat4 objectMatrix = [&]
     {
          const glm::vec3 tilePosition
            = glm::vec3(m_mouse_positions.down_pixel, 0.f) * scale
              * static_cast<float>(framebuffer.scale());
     ...
     // ImGuizmo::SetGizmoSizeClipSpace(selections->get<ConfigKey::DrawSwizzle>()
     // ? 0.021f : 0.15f);
           ...
           glm::value_ptr(objectMatrix)))
     {
          ...
          const auto      relative_pos
            = glm::vec2(newTilePosition.x, newTilePosition.y);
          ...
          if (m_mouse_positions.mouse_moved)
          {
               ...
               t_map_sprite->begin_multi_frame_working(
                 fmt::format(
                   "ImGuizmo {}, {}",
     ...
     else if (m_translation_in_progress && !ImGuizmo::IsUsing())
     {
          ...
          const glm::ivec2 delta
            = m_mouse_positions.down_pixel - m_location_backup;
          std::string history_entry = fmt::format(
            "ImGuizmo {} ({}, {})",
            ICON_FA_ARROWS_UP_DOWN_LEFT_RIGHT,
            delta.x,
            delta.y);
