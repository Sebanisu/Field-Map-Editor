...
class [[nodiscard]] import
{
   ...
   public:
     import();
     void render() const;
     [[nodiscard]] open_viii::graphics::background::Map::variant_tile                  &
     ...
     {
          const auto max_texture_id_tile
            = (std::ranges::max)(tiles,
                                 {},
                                 [](const auto &tile)
                                 { return tile.texture_id(); });
          const auto max_source_y_tile
            = (std::ranges::max)(tiles
                                   | std::ranges::views::filter(
                                     [&max_texture_id_tile](const auto &tile)
                                     {
                                          return tile.texture_id()
                                                 == max_texture_id_tile
                                                      .texture_id();
          ...
          const auto next_source_y
            = static_cast<uint8_t>((tile_y + 1) % tile_size_px_unsigned);
          const std::uint8_t next_texture_page
            = tile_y + 1 == tile_size_px_unsigned
                ? max_texture_id_tile.texture_id() + 1
                : max_texture_id_tile.texture_id();
