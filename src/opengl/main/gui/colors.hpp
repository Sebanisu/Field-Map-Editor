...
struct [[nodiscard]] color
{
     ...
     static constexpr auto max_f
       = static_cast<float_t>((std::numeric_limits<std::uint8_t>::max)());
     ...
     static constexpr auto min_f
       = static_cast<float_t>((std::numeric_limits<std::uint8_t>::min)());
     ...
     static constexpr auto max_i
       = static_cast<int_t>((std::numeric_limits<std::uint8_t>::max)());
  ...
  && __cpp_lib_saturation_arithmetic >= 202311L
     ...
     /**
      * @brief Constructs a color from integral RGB components with default
      * alpha.
      *
      * @tparam int_t An integral type.
      * @param in_r Red component.
  ...
  && __cpp_lib_saturation_arithmetic >= 202311L
     ...
     /**
      * @brief Constructs a color from floating-point RGB components with
      * default alpha.
      *
      * @tparam float_t A floating-point type.
      * @param in_r Red component (0.0-1.0).
      ...
      *
      * If the value is positive, the color will become brighter. If negative,
      * the color will become darker.
      *
      * @param fadeValue A float that determines the amount of fade. Positive
      * values brighten the color, negative values darken it.
      */
     [[nodiscard]] constexpr color fade(float fadeValue) const noexcept
     {
          // Clamp the fadeValue to avoid overflowing the color channels
          const float factor = 1.0f + fadeValue;// The factor by which the color
                                                // channels will be adjusted
          return { static_cast<std::uint8_t>(
                     std::clamp(r * factor, min_f<float>, max_f<float>)),
                   static_cast<std::uint8_t>(
                     std::clamp(g * factor, min_f<float>, max_f<float>)),
                   static_cast<std::uint8_t>(
                     std::clamp(b * factor, min_f<float>, max_f<float>)),
                   a };
     }
      ...
      *
      * If the value is positive, the color will become less transparent. If
      * negative, the color will become more transparent.
      *
      * @param fadeValue A float that determines the amount of fade. Positive
      * values brighten the color, negative values darken it.
      */
     [[nodiscard]] constexpr color fade_alpha(float fadeValue) const noexcept
     {
          ...
          return { r, g, b,
                   static_cast<std::uint8_t>(
                     std::clamp(a * factor, min_f<float>, max_f<float>)) };
     }
...
struct colors
{
     ...
     static constexpr color Peach
       = { 255, 153, 102 };//!< Gentle, warm orange variant
     ...
     static inline const color Button
       = color{ DarkStyle.Colors[ImGuiCol_Button] };
     ...
     // Green tint
     static constexpr color ButtonGreen
       = color{ ImVec4(0.26f, 0.98f, 0.26f, 0.40f) };
     static constexpr color ButtonGreenHovered
       = color{ ImVec4(0.26f, 0.98f, 0.26f, 1.00f) };
     static constexpr color ButtonGreenActive
       = color{ ImVec4(0.06f, 0.53f, 0.06f, 1.00f) };

     // Red tint
     static constexpr color ButtonRed
       = color{ ImVec4(0.98f, 0.26f, 0.26f, 0.40f) };
     static constexpr color ButtonRedHovered
       = color{ ImVec4(0.98f, 0.26f, 0.26f, 1.00f) };
     static constexpr color ButtonRedActive
       = color{ ImVec4(0.53f, 0.06f, 0.06f, 1.00f) };

     // Dark Blue tint
     static constexpr color ButtonDarkBlue
       = color{ ImVec4(0.26f, 0.26f, 0.59f, 0.40f) };
     static constexpr color ButtonDarkBlueHovered
       = color{ ImVec4(0.26f, 0.26f, 0.59f, 1.00f) };
     static constexpr color ButtonDarkBlueActive
       = color{ ImVec4(0.06f, 0.06f, 0.53f, 1.00f) };

     // Yellow tint
     static constexpr color ButtonYellow
       = color{ ImVec4(0.98f, 0.98f, 0.26f, 0.40f) };
     static constexpr color ButtonYellowHovered
       = color{ ImVec4(0.98f, 0.98f, 0.26f, 1.00f) };
     static constexpr color ButtonYellowActive
       = color{ ImVec4(0.53f, 0.53f, 0.06f, 1.00f) };

     // Orange tint
     static constexpr color ButtonOrange
       = color{ ImVec4(0.98f, 0.59f, 0.26f, 0.40f) };
     static constexpr color ButtonOrangeHovered
       = color{ ImVec4(0.98f, 0.59f, 0.26f, 1.00f) };
     static constexpr color ButtonOrangeActive
       = color{ ImVec4(0.53f, 0.26f, 0.06f, 1.00f) };

     // Pink tint
     static constexpr color ButtonPink
       = color{ ImVec4(0.98f, 0.26f, 0.59f, 0.40f) };
     static constexpr color ButtonPinkHovered
       = color{ ImVec4(0.98f, 0.26f, 0.59f, 1.00f) };
     static constexpr color ButtonPinkActive
       = color{ ImVec4(0.53f, 0.06f, 0.29f, 1.00f) };

     // Tan tint
     static constexpr color ButtonTan
       = color{ ImVec4(0.98f, 0.84f, 0.57f, 0.40f) };
     static constexpr color ButtonTanHovered
       = color{ ImVec4(0.98f, 0.84f, 0.57f, 1.00f) };
     static constexpr color ButtonTanActive
       = color{ ImVec4(0.53f, 0.48f, 0.29f, 1.00f) };
     ...
     {
          static const glm::vec4 K
            = glm::vec4(1.0f, 2.0f / 3.0f, 1.0f / 3.0f, 3.0f);
          // Replace GLSL c.xxx + K.xyz with explicit vector construction
          const glm::vec3 p = glm::abs(
            glm::fract(glm::vec3(c.x) + glm::vec3(K.x, K.y, K.z)) * 6.0f
            - glm::vec3(K.w));
          // Replace GLSL mix and clamp with GLM equivalents, ensuring correct
          // argument types
          return c.z
                 * glm::mix(
                   glm::vec3(K.x),
                   glm::clamp(p - glm::vec3(K.x), 0.0f, 1.0f),
                   c.y);
     }

     [[nodiscard]] static inline glm::vec4
       encode_uint_to_rgba(const unsigned int id)
     {
          ...
          const glm::vec3 temp_color = hsv2rgb(
            glm::vec3(
              hue,
              SATURATION,
              VALUE));// Explicitly round to integers for comparison
          // glm::vec3              rounded_color = glm::floor(temp_color *
          // 255.0f + 0.5f);
          /// return glm::vec4(rounded_color / 255.0f, 1.0f);// Keep in [0,1]
          /// for rendering
