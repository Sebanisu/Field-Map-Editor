...
struct key_value_data
{
     ...
     /// @details Represents the actual stored filename, usually parsed from a
     /// TOML. Unlike @ref ext, this is not constructed but read as-is.
     ...
     /// - 2-letter codes (e.g., `jp`, `en`, `de`) are common; the generic case
     /// is omitted.
     /// - 3-letter codes are also supported; the generic case is represented as
     /// `x`.
     std::optional<open_viii::LangT> language_code = {};


     /// @brief Optional palette index.
     /// @details In the original `.mim` textures, palettes were used with color
     /// keys (4-bit or 8-bit) to construct the final image. Multiple palettes
     /// could exist for a single texture, allowing reuse of the same pixel data
     /// with different color sets.
     std::optional<std::uint8_t>     palette       = {};

     /// @brief Optional texture page index.
     /// @details Original field textures were very wide but limited to 256px in
     /// height. Since the game engine could only handle ~256px wide textures,
     /// these were split into pages:
     /// - 8-bit color-keyed textures → 128px wide pages
     /// - 16-bit color variants (rare, no palette) → 64px wide pages
     ///
     /// In the modern pipeline, textures are normalized to 256x256. Depending
     /// on the bits-per-pixel, portions of the page may be half-empty or 3/4
     /// empty.
     std::optional<std::uint8_t>     texture_page  = {};

     /// @brief Optional pupu identifier.
     /// @details Represents a slice of the image containing a unique set of
     /// tiles and properties (e.g., animation ID, state, layer ID, blend mode).
     /// This helps prevent overlap when exporting or importing image data.
     /// In the newer TOML-based mode, multiple pupu_ids that are normally drawn
     /// together can be grouped for export, and later split back apart using a
     /// mask.
     ...
     struct keys
     {
          ...
          static constexpr std::string_view current_toml_path
            = { "current_toml_path" };
          static constexpr std::string_view ffnx_mod_path = { "ffnx_mod_path" };
          static constexpr std::string_view ffnx_direct_mode_path
            = { "ffnx_direct_mode_path" };
          static constexpr std::string_view ffnx_override_path
            = { "ffnx_override_path" };
          static constexpr std::string_view batch_input_path
            = { "batch_input_path" };
          static constexpr std::string_view batch_output_path
            = { "batch_output_path" };
          ...
          static constexpr std::string_view demaster_mod_path
            = { "demaster_mod_path" };
          static constexpr std::string_view ffnx_map = { "ffnx_map" };
          static constexpr std::string_view ffnx_multi_texture
            = { "ffnx_multi_texture" };
          static constexpr std::string_view ffnx_single_texture
            = { "ffnx_single_texture" };
          static constexpr std::string_view demaster = { "demaster" };
          static constexpr std::string_view ffnx_multi_texture_full
            = { "ffnx_multi_texture_full" };
          static constexpr std::string_view ffnx_single_texture_full
          ...
          static constexpr std::string_view field_3lang_main
            = { "field_3lang_main" };
          static constexpr std::string_view chara_3lang_main
            = { "chara_3lang_main" };
          static constexpr std::string_view eng
            = { "eng" };// open_viii::LangCommon::ENG
          ...
          static constexpr auto all_keys = std::to_array<std::string_view>(
            { selected_path, ff8_path, current_path, current_toml_path,
              ffnx_mod_path, ffnx_direct_mode_path, ffnx_override_path,
              // batch_input_path,
              // batch_output_path,
              field_name, ext, full_filename, field_prefix, letter_2_lang,
              letter_3_lang, palette, texture_page, pupu_id, demaster_mod_path,
              ffnx_map, ffnx_multi_texture, ffnx_single_texture, demaster,
              ffnx_multi_texture_full, ffnx_single_texture_full, demaster_full,
              field_main, field_lang, chara_main, chara_lang, field_3lang_main,
              chara_3lang_main });
          ...
          static constexpr std::string_view current_toml_path_tooltip
            = { "The current selected toml path." };
          static constexpr std::string_view ffnx_mod_path_tooltip
            = { "The base directory for FFNx mod files." };
          static constexpr std::string_view ffnx_direct_mode_path_tooltip
            = { "The directory for FFNx direct mode files, allowing file "
                "overrides. `.map` files would go here." };
          static constexpr std::string_view ffnx_override_path_tooltip
            = { "The FFNx override path for replacing game assets." };
          static constexpr std::string_view batch_input_path_tooltip
            = { "The folder containing files for batch processing." };
          static constexpr std::string_view batch_output_path_tooltip
            = { "The destination folder for processed batch output." };
          static constexpr std::string_view field_name_tooltip
            = { "The name of the current field or scene." };
          static constexpr std::string_view ext_tooltip
            = { "The file extension of the current resource." };
          static constexpr std::string_view full_filename_tooltip = {
               "The full filename including extension of the current resource."
          };
          static constexpr std::string_view field_prefix_tooltip
            = { "The prefix used for field-related assets." };
          static constexpr std::string_view letter_2_lang_tooltip
            = { "The two-letter language code (e.g., 'en' for English)." };
          static constexpr std::string_view letter_3_lang_tooltip
            = { "The three-letter language code (e.g., 'eng' for English)." };
          static constexpr std::string_view palette_tooltip
            = { "The color palette ID for textures." };
          static constexpr std::string_view texture_page_tooltip
            = { "The texture page ID used for rendering." };
          static constexpr std::string_view pupu_id_tooltip = {
               "The 32-bit hex number used for Pupu ID. This is for deswizzles."
          };
          static constexpr std::string_view demaster_mod_path_tooltip
            = { "The mod directory for Demaster, an FF8 modding tool." };

          static constexpr std::string_view ffnx_map_tooltip
            = { "Path layout for FFNx map data in direct mode:\n"
                "{ffnx_direct_mode_path}/field/mapdata/{field_prefix}/"
          ...
          static constexpr std::string_view ffnx_multi_texture_tooltip
            = { "Path layout for FFNx multi-texture field assets:\n"
                "{ffnx_mod_path}/field/mapdata/{field_prefix}/{field_name}/"
          ...
          static constexpr std::string_view ffnx_single_texture_tooltip
            = { "Path layout for FFNx single-texture field assets:\n"
                "{ffnx_mod_path}/field/mapdata/{field_name}/"
          ...
          static constexpr std::string_view demaster_tooltip
            = { "Path layout for field background textures in Demaster:\n"
                "{demaster_mod_path}/textures/field_bg/{field_prefix}/"
                "{field_name}/"
          ...
          static constexpr std::string_view field_lang_tooltip
            = { "Path for language-specific field assets:\n"
                "field/mapdata/{field_prefix}/{field_name}/"
                "{field_name}{_{2_letter_lang}}{ext}" };
          ...
          static constexpr std::string_view field_3lang_main_tooltip
            = { "Path for three-letter language-specific field assets:\n"
                "{3_letter_lang}/FIELD/mapdata/{field_prefix}/{field_name}/"
                "{field_name}{ext}" };
          ...
          static constexpr auto all_tooltips = std::to_array<std::string_view>(
            { selected_path_tooltip, ff8_path_tooltip, current_path_tooltip,
              current_toml_path_tooltip, ffnx_mod_path_tooltip,
              ffnx_direct_mode_path_tooltip, ffnx_override_path_tooltip,
              // batch_input_path_tooltip,
              // batch_output_path_tooltip,
              field_name_tooltip, ext_tooltip, full_filename_tooltip,
              field_prefix_tooltip, letter_2_lang_tooltip,
              letter_3_lang_tooltip, palette_tooltip, texture_page_tooltip,
              pupu_id_tooltip, demaster_mod_path_tooltip, ffnx_map_tooltip,
              ffnx_multi_texture_tooltip, ffnx_single_texture_tooltip,
              demaster_tooltip, ffnx_multi_texture_full_tooltip,
              ffnx_single_texture_full_tooltip, demaster_full_tooltip,
              field_main_tooltip, field_lang_tooltip, chara_main_tooltip,
              chara_lang_tooltip, field_3lang_main_tooltip,
              chara_3lang_main_tooltip });
     };

     /**
      * @brief Replaces placeholder tags in a string with values from test data
      * or a selected path.
      *
      * This function scans the input string for tags in the following formats:
      * - `{key}`: Replaced directly with the corresponding value from
      * `test_data` or `selected_path`.
      * - `{prefix{key}suffix}`: If the key resolves to a non-empty value, it's
      * inserted between the prefix and suffix.
      *
      * The function uses a regular expression to identify these patterns:
      * @code
      *   R"(\{([^\{\}]+)\}|\{([^\{]*)\{([^\}]+)\}([^\}]*)\})"
      * @endcode
      * This matches either a simple `{key}` or a nested format with prefix and
      * suffix.
      *
      * Nested tags are supported and resolved recursively up to a maximum depth
      * of 10 layers to prevent infinite loops. After replacements, all slashes
      * in the resulting string are normalized for the platform (Windows or
      * Linux).
      *
      * @param keyed_string  The input string possibly containing tag
      * placeholders.
      * @param test_data     A key-value data source to resolve the values
      * associated with tags.
      * @param selections    A shared pointer to additional selection context
      * used by `test_data`.
      * @param selected_path The replacement value for the `{selected_path}`
      * tag. Defaults to "{ff8_path}".
      *
      * @return A new string where all recognized tags have been replaced with
      * their corresponding values.
      * @todo This function might move as this is an odd place to put it. Maybe
      * it should be apart of key_value_data.
      */
     ...
     /**
      * @brief Static variant of replace_tags. Doesn't access instance-specific
      * data.
      *
      * @param keyed_string  The input string with tags.
      * @param selections    Context for resolving dynamic tags like `ff8_path`,
      * etc.
     ...
     /**
      * @brief Attempts to resolve a key using selections first, then static
      * fallbacks.
      *
      ...
      * placeholders or tags to their corresponding values based on external
      * data.
      *
      * @param key The input string containing a tag to replace.
      * @param selections A shared pointer to a Selections object used for tag
      * resolution.
      * @return A new string with the tag(s) replaced using the given
      * selections.
      */
     ...
     /**
      * @brief Replaces a tag in the given key without using external
      * selections.
      *
      * This overload resolves tags found in the key using internal or default
      * mechanisms, without any additional context from a Selections object.
      *
      * @param key The input string containing a tag to replace.
      * @return A new string with the tag(s) replaced using default or internal
      * rules.
      */
   ...
   private:
      ...
      *
      * @return A string view representing the first two characters of the field
      * name.
      */
      ...
      *
      * @return A string view of the 2-letter language code, or an empty view if
      * not set.
      */
      ...
      *
      * Converts the stored language code to a 3-letter representation (e.g.,
      * "eng" for English). If no language code is set, the default "eng"
      * (English) is returned.
      *
     ...
     {
          if (language_code.has_value())
               return open_viii::LangCommon::to_string_3_char(
                 language_code.value());
          else
               return open_viii::LangCommon::to_string_3_char(
                 open_viii::LangT::en);
