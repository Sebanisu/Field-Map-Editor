  ...
  const std::size_t                                         index)
{
     ...
     return std::visit(
       make_visitor(
         [&](
           const is_tile auto &original_tile,
           const is_tile auto &working_tile) -> bool
         {
              std::string title
                = index == std::numeric_limits<size_t>::max()
                    ? fmt::format("{}", gui_labels::selected_tile_info)
                    : fmt::format(
                        "{}: {}", gui_labels::selected_tile_info, index);
              const auto pushpopid0 = glengine::ScopeGuard{ &ImGui::PopID };
              ImGui::PushID(title.data());
              // ImGui::SetItemAllowOverlap();
              const bool open = ImGui::CollapsingHeader(
                title.data(), ImGuiTreeNodeFlags_AllowOverlap);
              ImGui::SameLine(
                ImGui::GetContentRegionAvail().x - 20);// adjust for alignment
              ...
              if (ImGui::BeginTable("table_tile_info", 2))
              {
                   ...
                   ImGui::TableSetupColumn(
                     "Key", ImGuiTableColumnFlags_WidthFixed);
                   ImGui::TableSetupColumn(
                     "Value", ImGuiTableColumnFlags_WidthStretch);

                   const auto the_end_table
                     = glengine::ScopeGuard([]() { ImGui::EndTable(); });
                   const std::string original_tile_string
                     = fmt::format("{}", original_tile);
                   const std::string working_tile_string
                     = fmt::format("{}", working_tile);
                   auto split_original_tile = split_lines(original_tile_string);
                   auto split_working_tile  = split_lines(working_tile_string);
                   auto zipped_tile_info
                     = std::views::zip(split_working_tile, split_original_tile);
                   for (auto [new_line, old_line] : zipped_tile_info)
                   {
                        auto split_kv = [](std::string_view line)
                        ...
                        new_val = new_val.starts_with(' ') ? new_val.substr(1)
                                                           : new_val;
                        old_val = old_val.starts_with(' ') ? old_val.substr(1)
                                                           : old_val;
              ...
              options_with_size.size
                = { width * tile_scale, width * tile_scale };
