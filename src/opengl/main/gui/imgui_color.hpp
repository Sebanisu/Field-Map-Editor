...
inline static constexpr number_type bitwise_or(number_type first, rest_number_type... rest)
{
     return static_cast<number_type>(static_cast<std::uint32_t>(first) | (static_cast<std::uint32_t>(rest) | ...));
}
template<is_enum_or_integral number_type, is_enum_or_integral... rest_number_type>
inline static constexpr number_type bitwise_and(number_type start, rest_number_type... rest)
{
...
inline static constexpr ImU32 imgui_color32(std::uint8_t red, std::uint8_t green, std::uint8_t blue, std::uint8_t alpha)
{
