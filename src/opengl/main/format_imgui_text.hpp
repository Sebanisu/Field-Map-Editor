...
inline const auto format_imgui_text
  = []<std::size_t sizeT = 0x3FFU, typename... T>(
      ...
      T &&...items)
{
     ...
     const auto              it = fmt::vformat_to_n(
       ...
       fmt::make_format_args(
         std::forward<decltype(std::as_const(items))>(
           std::as_const(items))...));
     *it.out = '\0';
...
inline const auto format_imgui_bullet_text
  = []<std::size_t sizeT = 0x3FFU, typename... T>(
      fmt::format_string<T...> fmt,
      T &&...items)
{
     ...
     const auto              it = fmt::vformat_to_n(
       ...
       fmt::make_format_args(
         std::forward<decltype(std::as_const(items))>(
           std::as_const(items))...));
     *it.out = '\0';
     ImGui::BulletText("%s", std::data(buffer));
     // it seems ImGui::Text is coping the values into it's own buffer.
};

inline const auto format_imgui_wrapped_text
  = []<std::size_t sizeT = 0x3FFU, typename... T>(
      fmt::format_string<T...> fmt,
      T &&...items)
{
