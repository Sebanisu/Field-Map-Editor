...
#define TILE_OPERATION(STRING, FUNCTION)                                     \
     ...
     {                                                                       \
          ...
          auto operator<=>(const CONCAT(                                     \
            STRING,                                                          \
            Match) &) const                                                  \
            = default;                                                       \
          bool operator==(const ValueT &that) const                          \
          {                                                                  \
               return m_value == that;                                       \
          }                                                                  \
     ...
     concept CONCAT(has_with_, FUNCTION) = requires(                         \
       std::remove_cvref_t<TileT> tile, CONCAT(STRING, T) < TileT > value) { \
     ...
     {                                                                       \
          using value_type = CONCAT(                                         \
            STRING,                                                          \
            T)<TileT>;                                                       \
          using get         = STRING;                                        \
          using get_default = CONCAT(                                        \
            STRING,                                                          \
            DefaultValue);                                                   \
          constexpr CONCAT(                                                  \
            STRING,                                                          \
          ...
          value_type            current = {};                                \
          constexpr static bool read_only                                    \
            = !CONCAT(has_with_, FUNCTION)<TileT>;                           \
          using transform_with = CONCAT(                                     \
            With,                                                            \
            STRING)<value_type>;                                             \
          using match_with = CONCAT(                                         \
            STRING,                                                          \
            Match)<value_type>;                                              \
     }
