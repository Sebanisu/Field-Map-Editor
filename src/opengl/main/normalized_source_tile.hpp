 ...
 *
 * This class defines a normalized representation of a source tile with texture, palette, and positional data.
 * It includes properties for layer identification, blending mode, and animation details. It excludes:
 * - **Destination X, Y, Z**: These are excluded since similar/duplicate tiles do not rely on specific destination coordinates.
 */
struct normalized_source_tile
{
   public:
      ...
      *
      * @tparam Tile The type of the tile object, requiring methods to access properties like texture ID, source position, etc.
     ...
     /**
      * @brief Assigns values from a generic tile object to this normalized tile.
      *
 ...
 *
 * This class defines a representation of an animated tile, focusing on key properties like texture, palette,
 * source position, layer, and animation ID. It excludes several properties that are unnecessary or inconsistent
 * for animations:
 *
 * - **Destination X, Y, Z**: These are excluded since animated tiles do not rely on specific destination coordinates.
 * - **Animation State**: Removed because only the animation ID is used to distinguish animations;
 *   the state is not relevant for this representation.
 * - **Blend Mode and Blend Other**: Excluded as some animations did not have consistent blends.
 * - **Palette for Animated Tiles**: When an animation ID is present, the palette is set to a unused (maximum value)
 *   because some animations have frames with different palettes.
 */
struct normalized_source_animated_tile
{
   public:
     TexIdBuffer         m_tex_id_buffer         = {};///< Buffer holding texture ID, depth, and draw flags (blending excluded).
     PaletteID           m_palette_id            = {};///< Identifier for the palette (some animations use a default palette).
     Point<std::uint8_t> m_source_xy             = {};///< Source position (x, y) of the tile within the texture.
     LayerID             m_layer_id              = {};///< Identifier for the layer this tile belongs to.
     std::uint8_t        m_animation_id          = (std::numeric_limits<std::uint8_t>::max)();///< Animation ID (default: no animation).
     ...
     constexpr auto operator<=>(const normalized_source_animated_tile &) const = default;


     /**
      * @brief Constructs a normalized animated tile from a `normalized_source_tile`.
      *
      * @param tile The source tile to initialize this animated tile from.
      */
     constexpr normalized_source_animated_tile(const normalized_source_tile &tile)
     ...
     constexpr normalized_source_animated_tile(const is_tile auto &tile)
       : m_tex_id_buffer{ TexIdBuffer{}.with_id(tile.texture_id()).with_depth(tile.depth()).with_draw(tile.draw()) }
     ...
     /**
      * @brief Assigns values from a `normalized_source_tile` to this animated tile.
      *
      * @param tile The source tile to assign from.
      * @return Reference to the updated animated tile.
      */
     constexpr normalized_source_animated_tile &operator=(const normalized_source_tile &tile)
     {
          m_tex_id_buffer = tile.m_tex_id_buffer.with_blend(0);
          m_palette_id    = tile.m_animation_id != (std::numeric_limits<std::uint8_t>::max)()
                              ? PaletteID{}.with_id((std::numeric_limits<std::uint8_t>::max)())
                              : tile.m_palette_id;
          m_source_xy     = tile.m_source_xy;
          m_layer_id      = tile.m_layer_id;
      ...
      * @return Reference to the updated animated tile.
      */
     constexpr normalized_source_animated_tile &operator=(const is_tile auto &tile)
     {
          m_tex_id_buffer = TexIdBuffer{}.with_id(tile.texture_id()).with_depth(tile.depth()).with_draw(tile.draw());
          m_palette_id    = tile.animation_id() != (std::numeric_limits<std::uint8_t>::max)()
                              ? PaletteID{}.with_id((std::numeric_limits<std::uint8_t>::max)())
     ...
     /// @brief Default copy constructor.
     constexpr normalized_source_animated_tile(const normalized_source_animated_tile &)                = default;

     /// @brief Default copy assignment operator.
     constexpr normalized_source_animated_tile &operator=(const normalized_source_animated_tile &)     = default;

     /// @brief Default move constructor.
     constexpr normalized_source_animated_tile(normalized_source_animated_tile &&) noexcept            = default;

     /// @brief Default move assignment operator.
     constexpr normalized_source_animated_tile &operator=(normalized_source_animated_tile &&) noexcept = default;
};
}// namespace open_viii::graphics::background
