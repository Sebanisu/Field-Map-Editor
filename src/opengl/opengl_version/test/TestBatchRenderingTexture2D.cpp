    ...
    std::filesystem::current_path() / "res" / "shader" / "basic3.shader")
{
  ...
  vertices += CreateQuad(glm::vec3{ -4.0F, 0.F, 0.F } + offset, colors[0], 1)
              + CreateQuad(glm::vec3{ 0.F, 0.F, 0.F } + offset, colors[1], 2)
              + CreateQuad(glm::vec3{ 4.F, 0.F, 0.F } + offset, colors[2], 3);
