...
const std::string &ff_8::Upscales::string() const
{
     ...
     {
          return m_paths.get(static_cast<std::size_t>(m_current))
            ->ref<std::string>();
     }
...
static void check_paths(std::vector<std::string> &paths)
{
     if (!std::ranges::empty(paths))
     {
          ...
          using CountT = std::iter_difference_t<
            decltype(std::filesystem::directory_iterator(path, ec))>;
          CountT max_count = {};
          for (const auto &sub_path : recursive_dir)
          {
               ...
               {
                    auto counting_path
                      = std::filesystem::directory_iterator(sub_path, ec);
                    if (ec)
                    {
                    ...
                    const auto count = std::ranges::count_if(
                      ...
                      [&handle_error, &ec](const auto &item)
                      {
                           bool is_regular_file
                             = std::filesystem::is_regular_file(item, ec);
      ...
      [&]() -> toml::array
      {
           if (!config->contains(upscale_paths_vector))
           {
                ...
                config->insert_or_assign(
                  upscale_paths_vector, std::move(paths_array));
                config.save();
           }
