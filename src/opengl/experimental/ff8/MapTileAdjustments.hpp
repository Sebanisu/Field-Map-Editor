...
class MapTileAdjustments
{
   ...
   public:
       ...
       operator()(const TileT &tile, bool &changed, std::size_t i, const std::optional<glengine::SubTexture> &sub_texture) const
     {
          ...
          ImGui::SameLine();
          {
               const auto disabled = glengine::ImGuiDisabled(!m_map_history.undo_enabled());
          ...
          inputs_read_only(tile, static_cast<int>(i), static_cast<int>(static_cast<uint32_t>(sub_texture->id())));
          return visit_state;
     }
   ...
   private:
     ...
     void inputs_read_only(const auto &tile, const int index, const int id) const
     {
          ...
          // const_cast
          {
               const std::array<int, 2> tile_dims = { static_cast<int>(tile.width()), static_cast<int>(tile.height()) };
               ImGui::InputInt2("Tile Dimensions", const_cast<int *>(tile_dims.data()), ImGuiInputTextFlags_ReadOnly);
          }
          {
               const std::string index_str = fmt::format("{}", index);
               ImGui::InputText("Index", const_cast<char *>(index_str.data()), index_str.size(), ImGuiInputTextFlags_ReadOnly);
          }
          {
          ...
          }
          {
               ...
               ImGui::InputText("Raw Hex", const_cast<char *>(hex.data()), hex.size(), ImGuiInputTextFlags_ReadOnly);
          }
     }
     template<typename TileT>
     ...
     bool generic_slider_int(GroupT &group, float width = {}, int step = 1, std::optional<int> override_max = std::nullopt) const
     {
          ...
          if constexpr (!GroupT::read_only)
          {
               ...
               if (ImGui::SliderInt(
                     ...
                     override_max ? *override_max : static_cast<int>(group.max_value) / step,
                     {},
                     ImGuiSliderFlags_AlwaysClamp))
               {
                    group.current = static_cast<typename GroupT::value_type>(current_int * step);
                    return true;
               }
          }
          else
          {
               (void)ImGui::InputText(fmt::format("{}", group.current), ImGuiInputTextFlags_ReadOnly);
          }
          return false;
     }
     template<typename TileT>
     void slider_int_2_source_xy(const TileT &tile, bool &changed, const int current_bpp_selection) const
     {
          ...
          const std::pair<float, float> item_width     = generate_inner_width(2, checkbox_width);
          checkbox_tool_tip("##matching source_xy", "Matching Source X and Source Y", m_matching.source_xy);
          checkbox_tool_tip("##matching source_x", "Matching Source X", m_matching.source_x);
          auto x = SourceXGroup(tile);
          if (generic_slider_int(x, item_width.first, tile.width(), (static_cast<int>(std::pow(2, (2 - current_bpp_selection) + 2) - 1))))
          {
               changed       = true;
               const auto op = [&](TileT &new_tile) { new_tile = new_tile.with_source_x(x.current); };
               m_map_history.copy_working_perform_operation(tile, m_matching, op);
          }
          ImGui::SameLine(0, ImGui::GetStyle().ItemInnerSpacing.x);
          checkbox_tool_tip("##matching source_y", "Matching Source Y", m_matching.source_y);
          auto y = SourceYGroup(tile);
          if (generic_slider_int(y, item_width.second, tile.height()))
          {
               changed       = true;
               const auto op = [&](TileT &new_tile) { new_tile = new_tile.with_source_y(y.current); };
               m_map_history.copy_working_perform_operation(tile, m_matching, op);
          }
          ImGui::SameLine(0, ImGui::GetStyle().ItemInnerSpacing.x);
          ImGui::Text("%s", fmt::format("Source Pos: ({}, {})", x.current, y.current).c_str());
     ...
     void slider_int_3_xyz(const TileT &tile, bool &changed) const
     {
          ...
          const std::pair<float, float> item_width     = generate_inner_width(3, checkbox_width);
          checkbox_tool_tip("##matching xyz", "Matching Destination X, Destination Y and Destination Z", m_matching.xyz);
          checkbox_tool_tip("##matching xy", "Matching Destination X and Destination Y", m_matching.xy);
          checkbox_tool_tip("##matching x", "Matching Destination X", m_matching.x);
          auto x = XGroup{ tile };
          if (generic_slider_int(x, item_width.first, tile.width()))
          {
               changed = true;
               m_map_history.copy_working_perform_operation(
                 tile, m_matching, [&](TileT &new_tile) { new_tile = new_tile.with_x(x.current); });
          }
          ImGui::SameLine(0, ImGui::GetStyle().ItemInnerSpacing.x);
          checkbox_tool_tip("##matching y", "Matching Destination Y", m_matching.y);
          auto y = YGroup{ tile };
          if (generic_slider_int(y, item_width.first, tile.width()))
          {
               changed = true;
               m_map_history.copy_working_perform_operation(
                 tile, m_matching, [&](TileT &new_tile) { new_tile = new_tile.with_y(y.current); });
          }
          ImGui::SameLine(0, ImGui::GetStyle().ItemInnerSpacing.x);
          checkbox_tool_tip("##matching z", "Matching Destination Z", m_matching.z);
          auto z = ZGroup{ tile };
          if (generic_slider_int(z, item_width.second, tile.width()))
          {
               changed = true;
               m_map_history.copy_working_perform_operation(
                 tile, m_matching, [&](TileT &new_tile) { new_tile = new_tile.with_z(z.current); });
     ...
     void combo_blend_modes(const TileT &tile, bool &changed) const
     {
          ...
          const float checkbox_width = get_checkbox_width(1U);
          checkbox_tool_tip("##matching blend mode", "Matching Blend Mode", m_matching.blend_mode);
          if (glengine::GenericCombo("Blend Mode", current_blend_mode_selection, blend_mode_str, checkbox_width))
          {
     ...
     void slider_int_layer_id(const TileT &tile, bool &changed) const
     {
          ...
          const float width          = ImGui::CalcItemWidth();
          const auto  pop_width      = glengine::ImGuiPushItemWidth(width - checkbox_width);
          checkbox_tool_tip("##matching layer_id", "Matching Layer ID", m_matching.layer_id);
          if (ImGui::SliderInt("Layer ID", &layer_id, LayerIdT<TileT>{}, LayerIdT<TileT>{ 0b0111'1111U }, {}, ImGuiSliderFlags_AlwaysClamp))
          {
     ...
     void slider_int_texture_page_id(const TileT &tile, bool &changed) const
     {
          ...
          const float width           = ImGui::CalcItemWidth();
          const auto  pop_width       = glengine::ImGuiPushItemWidth(width - checkbox_width);
          checkbox_tool_tip("##matching texture_page_id", "Matching Texture Page ID", m_matching.texture_id);
          if (ImGui::SliderInt(
                "Texture Page ID",
                &texture_page_id,
                TextureIdT<TileT>{},
     ...
     void slider_int_palette_id(const TileT &tile, bool &changed) const
     {
          ...
          const float width          = ImGui::CalcItemWidth();
          const auto  pop_width      = glengine::ImGuiPushItemWidth(width - checkbox_width);
          checkbox_tool_tip("##matching palette_id", "Matching Palette ID", m_matching.palette_id);
          int palette_id = static_cast<int>(tile.palette_id());
          if (ImGui::SliderInt(
                "Palette ID", &palette_id, PaletteIdT<TileT>{}, PaletteIdT<TileT>{ 0b1111U }, {}, ImGuiSliderFlags_AlwaysClamp))
     ...
     void slider_int_blend_other(const TileT &tile, bool &changed) const
     {
          ...
          const float checkbox_width = get_checkbox_width(1U);
          const float width          = ImGui::CalcItemWidth();
          checkbox_tool_tip("##matching blend other", "Matching Blend Other", m_matching.blend);
          const auto pop_width = glengine::ImGuiPushItemWidth(width - checkbox_width);
          if (ImGui::SliderInt("Blend Other", &blend, BlendT<TileT>{}, BlendT<TileT>{ 0b0011U }, {}, ImGuiSliderFlags_AlwaysClamp))
          {
               changed = true;
               m_map_history.copy_working_perform_operation(
                 tile, m_matching, [&](TileT &new_tile) { new_tile = new_tile.with_blend(static_cast<BlendT<TileT>>(blend)); });
     ...
     void slider_int_2_animation(const TileT &tile, bool &changed) const
     {
          ...
          checkbox_tool_tip("##matching animation_id", "Matching Animation ID", m_matching.animation_id);
          const std::pair<float, float> item_width = generate_inner_width(2, checkbox_width);
          {
               const auto disabled  = glengine::ImGuiDisabled(!has_with_animation_id<TileT>);
          ...
          ImGui::SameLine(0, ImGui::GetStyle().ItemInnerSpacing.x);
          checkbox_tool_tip("##matching animation_state", "Matching Animation State", m_matching.animation_state);
          {
               const auto disabled  = glengine::ImGuiDisabled(!has_with_animation_state<TileT>);
          ...
          ImGui::SameLine(0, ImGui::GetStyle().ItemInnerSpacing.x);
          ImGui::Text("%s", fmt::format("Animation: ({}, {})", animation_id, animation_state).c_str());
     }
};
}// namespace ff_8
