...
class MapFilters
{
     ...
     MapFilters(const MapHistory &map)
       ...
       , m_disabled(visit(
           ...
           [](
             ...
             auto &&b_tiles)
           {
                return std::ranges::empty(f_tiles)
                       || std::ranges::empty(b_tiles);
           }))
     {
     }
          ...
          bool ret_changed = false;
          {
               ...
               if (ImGui::Begin("Map Filters"))
               {
                    ...
                    static const auto common
                      = [](
                          ...
                          std::ranges::random_access_range auto
                            &&possible_value_range,
                          std::ranges::random_access_range auto
                            &&possible_value_string_range,
                          std::ranges::random_access_range auto
                                 &&used_value_range,
                          uint32_t line_count)
                    {
                         ...
                         if (ImGui::CollapsingHeader(label))
                         {
                              ...
                              const auto boolsent
                                = std::ranges::end(bool_range);
                              auto current_value
                                = std::ranges::cbegin(possible_value_range);
                              auto current_string = std::ranges::cbegin(
                                possible_value_string_range);

                              static constexpr const char *clicked_pattern
                                = "Clicked {}";
                                            ...
                                            (void)++current_string)
                              {
                                   ...
                                   auto size
                                     = ImGui::CalcTextSize(string.c_str());
                                   bool same_line
                                     = line_count > 0 && i % line_count != 0;
                                   if (same_line)
                                        ImGui::SameLine();
                    ...
                    const auto changes = std::array{
                         ...
                         common_unique(
                           "Blend Other", m_unique_tile_values.blend_other, 8),

                         common_unique("Z", m_unique_tile_values.z, 4),

                         common_unique(
                           "Layer ID", m_unique_tile_values.layer_id, 8),

                         common_unique(
                           "Texture Page ID",
                           m_unique_tile_values.texture_page_id,
                           8),

                         common_unique(
                           "Animation ID",
                           m_unique_tile_values.animation_id,
                           8),

                         common_unique(
                           "Animation Frame",
                           m_unique_tile_values.animation_state,
                           8),
          ...
          return std::ranges::all_of(
            std::array{ filter(
                        ...
                        filter(
                          ...
                          m_unique_tile_values.animation_state.enable(),
                          m_unique_tile_values.animation_state.values()),
                        filter(
                          tile.layer_id(),
                          m_unique_tile_values.layer_id.enable(),
                          m_unique_tile_values.layer_id.values()),
   ...
   private:
     ...
     {
          assert(
            std::ranges::size(bool_range) == std::ranges::size(value_range));
