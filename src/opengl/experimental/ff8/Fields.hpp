...
class Fields
{
   ...
   private:
     ...
     mutable std::chrono::steady_clock::time_point   m_start_time    = std::chrono::steady_clock::now();
     mutable int                                     m_current_index = {};
     Archive                                         m_archive       = {};
     mutable std::vector<std::string>                m_map_data      = {};
     mutable open_viii::archive::FIFLFS<false>       m_field         = {};
     mutable std::chrono::steady_clock::time_point   m_end_time      = std::chrono::steady_clock::now();
};
