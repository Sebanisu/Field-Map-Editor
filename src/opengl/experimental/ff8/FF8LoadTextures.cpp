     ...
     for (const auto texture_page : texture_pages)
     {
          ...
          rdt.futures.emplace_back(
            std::async(
              ...
              process,
              current_file,
              &rdt.textures->at(index)));
     }
     for (const auto texture_page : texture_pages)
          for (const auto palette : palettes)
          {
               const auto current_file = swizzle_path
                                         / fmt::format(
                                           "{}_{}_{}.png",
                                           current_file_prefix,
                                           +texture_page,
                                           +palette);
               const auto index
                 = texture_page + TexturePageCount * (palette + 1U);
               if (!std::filesystem::exists(current_file))
                    continue;
               spdlog::debug("Loading Texture: {}", current_file.string());
               rdt.futures.emplace_back(
  ...
  ff_8::LoadTextures(const open_viii::graphics::background::Mim &mim)
{
     ...
     {
          auto r = glengine::DelayedTexturesData{
               .colors = in_mim.get_colors<glm::vec<4, std::uint8_t>>(
                 in_bpp, in_palette),
                ...
                glengine::Texture *in_out) -> glengine::DelayedTexturesData
              {
                   auto r = glengine::DelayedTexturesData{
                        .colors = in_mim.get_colors<glm::vec<4, std::uint8_t>>(
                          {}, {}, true),
                        .width
                        = static_cast<std::int32_t>(in_mim.get_width({}, true)),
                        .height
                        = static_cast<std::int32_t>(in_mim.get_height(true)),
                        .out = in_out
                   };
                   glengine::Texture::flip(r.colors, r.width);
                   return r;
