...
glengine::DelayedTextures<ff_8::UpscaleTextureCount> ff_8::LoadTextures(const std::filesystem::path &swizzle_path)
{
     ...
     auto process = [](const std::filesystem::path file_path, glengine::Texture *in_out) -> glengine::DelayedTexturesData {
          ...
          auto png = std::unique_ptr<stbi_uc, decltype(deleter)>(stbi_load(file_path.string().c_str(), &x, &y, &channels, 4));
          auto r   = glengine::DelayedTexturesData{ .path = file_path, .width = x, .height = y, .out = in_out };
          r.colors.resize(static_cast<std::size_t>(x * y));
          std::memcpy(r.colors.data(), png.get(), static_cast<std::size_t>(x * y) * 4U);
          return r;
     };
     auto        texture_pages       = std::views::iota(std::uint8_t{}, std::uint8_t{ 13 });
     auto        palettes            = std::views::iota(std::uint8_t{}, std::uint8_t{ 16 });
     std::string current_file_prefix = swizzle_path.stem().string();
     for (const auto texture_page : texture_pages)
     {
          const auto current_file = swizzle_path / fmt::format("{}_{}.png", current_file_prefix, +texture_page);
          const auto index        = texture_page;
          if (!std::filesystem::exists(current_file))
               continue;
          spdlog::debug("Loading Texture: {}", current_file.string());
          rdt.futures.emplace_back(std::async(std::launch::async, process, current_file, &rdt.textures->at(index)));
     }
     for (const auto texture_page : texture_pages)
          for (const auto palette : palettes)
          {
               const auto current_file = swizzle_path / fmt::format("{}_{}_{}.png", current_file_prefix, +texture_page, +palette);
...
glengine::DelayedTextures<35U> ff_8::LoadTextures(const open_viii::graphics::background::Mim &mim)
{
     ...
     for (std::size_t bpp_offset{}; const open_viii::graphics::BPPT bpp : bpps)
     {
          ...
          {
               spdlog::debug("Loading BPP {}, Palette {} Texture", 1U << (bpp_offset + 2U), palette);
               rdt.futures.emplace_back(
                 std::async(std::launch::async, process, mim, bpp, palette, &rdt.textures->at(bpp_offset * 16 + palette)));
          }
          ++bpp_offset;
     }
     {
          ...
          rdt.futures.emplace_back(std::async(std::launch::async, process, mim, 16_bpp, std::uint8_t{ 0 }, &rdt.textures->at(2 * 16)));

          spdlog::debug("Loading BPP 24 Texture");
          rdt.futures.emplace_back(std::async(std::launch::async, process, mim, 24_bpp, std::uint8_t{ 0 }, &rdt.textures->at(2 * 16 + 1)));
     }
     {
