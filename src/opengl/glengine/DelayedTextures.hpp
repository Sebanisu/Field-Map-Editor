...
struct DelayedTextures
{
     ...
     mutable std::unique_ptr<std::array<Texture, sizeT>>   textures{ std::make_unique<std::array<Texture, sizeT>>() };
          ...
          for (std::future<DelayedTexturesData> &future : futures)
          {
               if (future.valid() && future.wait_for(std::chrono::seconds(0)) == std::future_status::ready)
               {
                    ...
                    *rd.out                = Texture(rd.colors, rd.width, rd.height);
                    spdlog::debug("Finished Loading Texture: {:>2}\t{}", std::distance(textures->data(), rd.out), rd.path.string());
                    return true;
               }
          }
          remove_invalid();
