...
void HistogramBuffer::read_back(std::vector<GLuint> &data)
{
     ...
     // GlCall {} (glGetBufferSubData,GL_SHADER_STORAGE_BUFFER, 0, m_count *
     // sizeof(GLuint), data.data());
     void *ptr = GlCall{}(
       glMapBufferRange, GL_SHADER_STORAGE_BUFFER, 0,
       static_cast<GLsizeiptr>(m_count * sizeof(GLuint)), GL_MAP_READ_BIT);
     if (ptr)
     {
...
void HistogramBuffer::reset() const
{
     ...
     GlCall{}(
       ...
       static_cast<GLsizeiptr>(m_count * sizeof(GLuint)), zero_data.data());
     GlCall{}(glBindBuffer, GL_SHADER_STORAGE_BUFFER, 0);
     if (glGetError() != GL_NO_ERROR)
     {
...
GLuint HistogramBuffer::create(size_t count)
{
     ...
     GlCall{}(
       ...
       static_cast<GLsizeiptr>(count * sizeof(GLuint)), init_data.data(),
       GL_DYNAMIC_COPY);
     GlCall{}(glBindBuffer, GL_SHADER_STORAGE_BUFFER, 0);
     if (glGetError() != GL_NO_ERROR)
     {
