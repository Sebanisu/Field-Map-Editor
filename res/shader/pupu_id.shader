...
layout(location = 1) out uint out_pupu_id;
...
// Texture array of RGBA8UI PupuID mask images
uniform highp usampler2D u_PupuIDMask[32];

void main()
{
  int index = int(v.texture);
  uvec4 texColor = texture(u_PupuIDMask[index], v.TexCoord);

  // Reconstruct 32-bit Pupu ID from RGBA channels
  uint pupu_id = texColor.r |
                 (texColor.g << 8u) |
                 (texColor.b << 16u) |
                 (texColor.a << 24u);
